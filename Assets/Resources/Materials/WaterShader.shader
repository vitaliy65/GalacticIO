    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float eyeDepth : TEXCOORD3; // Added to hold explicit linear eye depth
            };
            ...
            v2f vert (appdata v)
            {
                ...
                // Explicitly calculate camera eye depth (works for both Perspective & Ortho)
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                // Calculate linear depth dynamically based on camera projection mode
                float existingDepthLinear = 0;
                
                // unity_OrthoParams.w is 1.0 when orthographic, 0.0 when perspective
                if (unity_OrthoParams.w > 0.0)
                {
                    ...
                    #endif
                    // Remap from 0-1 to eye units using Near plane (y) and Far plane (z)
                    existingDepthLinear = _ProjectionParams.y + rawDepth * (_ProjectionParams.z - _ProjectionParams.y);
                }
                else
                {
                    // Standard perspective branch
                    existingDepthLinear = LinearEyeDepth(existingDepth01);
                }

                // Difference between the water's surface and the object behind it.
                ...
                // Retrieve the view-space normal of the surface behind the pixel
                float3 existingNormal = tex2Dproj(_CameraNormalsTexture, UNITY_PROJ_COORD(i.screenPosition));
                
                // Modulate the amount of foam we display based on the difference between normals
                ...
                // Distort the noise UV and offset it by time
